...
async function seed() {
  ...
  await upsertLead({
    ...
    returnReceiptStatus: "received_at_hub",
    returnResolution: "pending",
    returnPartName: "فلتر زيت",
    returnPackageName: "باكدج 20 ألف كم",
    returnInspectionNotes: "العميل أفاد أن الفلتر وصل بمقاس غير مطابق. القطعة في نقطة الاستلام وتحت المراجعة قبل اعتماد الاستبدال.",
    knowledgeNotes: "إذا تأكد عدم المطابقة مع رقم الشاسيه يتم اعتماد الاستبدال فورًا وإخطار مسؤول القطع لتجهيز البديل.",
    assignedEmployeeId: technicalUser.id,
    createdByUserId: salesUser.id,
    lastContactAt: now,
    nextFollowUpAt: tomorrowMorning,
    notes: "تم رفع الطلب من المبيعات بعد شكوى عميل بخصوص قطعة ضمن الباكدج، والخبير الفني مطلوب منه تأكيد المطابقة ثم تحديد القرار.",
  });
  ...
  await upsertLead({
    ...
    technicalActionMode: "contact_directly",
    transferDecision: "keep_with_technical",
    returnRequestType: "refund",
    returnStatus: "awaiting_customer_handover",
    returnReceiptStatus: "not_received",
    returnResolution: "pending",
    returnPartName: "تيل فرامل أمامي",
    returnPackageName: null,
    returnInspectionNotes: "العميلة ستسلّم القطعة غدًا لفحص حالتها قبل رفع القرار.",
    knowledgeNotes: "لا يعتمد الرد المالي إلا بعد فحص حالة العبوة والتأكد أن القطعة لم تُستخدم.",
    assignedEmployeeId: technicalUser.id,
    createdByUserId: adminUserId,
    lastContactAt: now,
    nextFollowUpAt: tomorrowAfternoon,
    notes: "مرتجع مباشر من عميلة تطلب رد قيمة القطعة، ويجب توثيق الاستلام أولًا ثم الرفع للإدارة إذا استوفت الشروط.",
  });
  ...
  await upsertLead({
    ...
    technicalPriority: "medium",
    technicalActionMode: "coordinate_with_workshop",
